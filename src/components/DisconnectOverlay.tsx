...
export function DisconnectOverlay({ player, isHost, onRemove }: DisconnectOverlayProps) {
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-blue-950/95 border-2 border-cyan-700/30 rounded-2xl p-8 max-w-sm w-full text-center flex flex-col items-center gap-4 shadow-2xl">
        <div className="w-14 h-14 rounded-full bg-red-900/40 border border-red-700/40 flex items-center justify-center">
          <span className="text-2xl">📶</span>
        </div>

        <div>
          <h2 className="text-cyan-200 font-bold text-lg">Conexão perdida</h2>
          <p className="text-cyan-400 font-black text-2xl mt-1">{player.name}</p>
        </div>

        <p className="text-blue-600/70 text-sm animate-pulse">
